...
class CGAN(object):
    ...
    def discriminator(self, x, y, is_training=True, reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            d1 = lrelu(bn(conv2d(d0, df_dim * 2, name='d_d1_conv'), is_training=is_training, scope='d_bn1'))
            d2 = lrelu(bn(conv2d(d1, df_dim * 4, name='d_d2_conv'), is_training=is_training, scope='d_bn2'))
            d3 = lrelu(bn(conv2d(d2, df_dim * 8, name='d_d3_conv'), is_training=is_training, scope='d_bn3'))
            d4 = linear(tf.reshape(d3, [-1, df_dim * 8 * 2 * 2]), 1, 'd_d4_lin')
    ...
    def generator(self, z, y, is_training=True, reuse=False):
        ...
        with tf.variable_scope("generator", reuse=reuse):
            ...
            """
            gf_dim = 128
            g0 = tf.reshape(linear(z, gf_dim*8*2*2, scope='g_fc1'), [-1, 2, 2, gf_dim*8])
            g0 = tf.nn.relu(bn(g0, is_training=is_training, scope='g_bn0'))
            g1 = tf.nn.relu(bn(deconv2d(g0, [self.batch_size, 4, 4, gf_dim * 4], name='g_dc1'), is_training=is_training,
                               scope='g_bn1'))
            g2 = tf.nn.relu(bn(deconv2d(g1, [self.batch_size, 7, 7, gf_dim * 2], name='g_dc2'), is_training=is_training,
                               scope='g_bn2'))
            g3 = tf.nn.relu(bn(deconv2d(g2, [self.batch_size, 14, 14, gf_dim], name='g_dc3'), is_training=is_training,
                               scope='g_bn3'))
            g4 = tf.nn.tanh(deconv2d(g3, [self.batch_size, 28, 28, 1], name='g_dc5'))
            print(z.shape, g0.shape, g1.shape, g2.shape, g3.shape, g4.shape)
            return g4
